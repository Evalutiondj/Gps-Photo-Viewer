...
class PhotoGPSViewer:
    ...
    def update_list_status(self):
        self.list_status.config(text=f"📊 {len(self.image_list)} ảnh")
    ...
    def on_search_change(self):
        ...
        self.list_status.config(text=f"🔍 Tìm thấy: {len(results)}/{len(source_list)}")
    ...
    def apply_filter(self, filter_type):
        ...
        self.list_status.config(text="⏳ Đang lọc...")
        self.root.update()
        
        if filter_type == 'has_gps':
            self.filtered_list = [p for p in self.image_list if self.get_gps_data_cached(p)]
            filter_name = "có GPS"
        elif filter_type == 'no_gps':
            self.filtered_list = [p for p in self.image_list if not self.get_gps_data_cached(p)]
            filter_name = "không có GPS"
        else:
            return
        
        self.is_filtered = True
        self.update_listbox(self.filtered_list)
        self.list_status.config(text=f"📍 Lọc: {len(self.filtered_list)} ảnh {filter_name}")
    ...
    def filter_by_camera(self):
        ...
        for p in self.image_list:
            data = self.get_gps_data_cached(p)
            if p in self.exif_cache:
                tags = self.exif_cache[p]
                make = str(tags.get('Image Make', '')).strip()
                model = str(tags.get('Image Model', '')).strip()
